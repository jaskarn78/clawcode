...
- [x] **POL-03**: Policy hot-reload — editing the policy file takes effect on the NEXT trigger evaluation without a daemon restart
...
- [x] **OBS-04**: Proactive turns and delegated task turns appear in the existing v1.7 trace tree with `causation_id`, `trigger_id`, and `task_id` metadata on the root span; `clawcode trace <causation_id>` CLI walks the entire chain (source event → trigger → turn → handoff → turn → ... → final result) across all involved agents
