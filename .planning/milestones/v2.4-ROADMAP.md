...
- [x] 91-02-PLAN.md — Conflict detection (sha256 pre-sync compare, per-file skip) + admin-clawdy bot-direct alert embed (SYNC-06)
...
- [x] 91-04-PLAN.md — `clawcode sync *` CLI subcommands (status, run-once, resolve, set-authoritative, reverse start/stop, finalize, translate-sessions) with drain-then-flip + 7-day rollback gate (SYNC-09, SYNC-10)
- [x] 91-05-PLAN.md — /clawcode-sync-status Discord slash (Phase 85 CONTROL_COMMANDS + EmbedBuilder blueprint) (SYNC-08)
- [x] 91-06-PLAN.md — fin-acquisition-cutover.md runbook extension (SSH provisioning, systemd install, cutover flip, 7-day rollback, observability) + exclude-filter regression test (SYNC-10 pin)
