...
status: v2.9-deployed-with-findings
stopped_at: "v2.9 DEPLOYED 2026-05-14 21:04 PDT (build 4e96c24, md5 77cc6cdb...) — 47 commits total. Phase 119 (A2A): Plan 01 deployed; FIND-119-A boot sentinel didn't fire (silent-path-bifurcation manifestation; code IS deployed, only proof-of-wiring broken). Phase 120 diagnostic SQL run: FIND-120-A LENGTH<=11 guard hypothesis FALSE (returns zero rows on real data); FIND-120-B DASH-04 producer dead (348 end_to_end spans on Admin Clawdy, ZERO with any latency metadata). Phase 121: shipped (sub-bug D completion gate + sub-bug B chunk seam, 108/108 tests, splitMessage no-seam confirmed). Phase 122: shipped (5 chokepoints wrapped, 36/36 tests, helper extended for nested-fence SC-3). Phase 123 soak: FIND-123-A transient mcp-server-mysql orphans on systemctl-restart cycle — 1-2 per restart, persist 4-5 min, reaper catches them at T+9min; steady-state clean. Variants B+C skipped per CONTEXT D-08. Phase 124: Plan 00 SDK probe + Plan 02 heartbeat decoupling shipped (20/20 tests). D-12 operator decision: forkSession + summary-prepend path chosen. Plans 01/03/04 unblocked (next session). Phase 125 unblocked — tiered retention builds on D-12 primitive. Three FIND-XX-A items filed for follow-up phases. Ramy-active hold: operator-cleared this deploy window. Next: write Phase 124 Plans 01/03/04 against D-12 design; write Phase 125 plans; execute 119 Wave 2; address FIND-119-A / 120-B / 123-A in follow-up phases."
