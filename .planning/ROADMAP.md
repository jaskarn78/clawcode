...
17. **Tool-latency methodology audit + parallel-tool-call leverage** — observed dashboard shows EVERY tool flagged `[SLOW]` including built-in Claude Code tools (`Read` 77s p50, `Edit` 37s p50, `Glob` 88s p50, `Bash` 86s p50, `Grep` 47s p50). These tools execute in milliseconds; the tens-of-seconds latency is the LLM round-trip around the tool call (cache-bust + output generation + extended-thinking on resume). Three deliverables:
    (a) **Trace-collector audit** — `src/performance/trace-collector.ts`: is `tool_call` measuring pure execution or full round-trip? Likely full round-trip per the data shape. Split into two metrics:
       - `tool_execution_ms` — from SDK dispatch to tool result returned (the milliseconds part)
       - `tool_roundtrip_ms` — full duration including LLM resume after tool result (the seconds part)
       - Dashboard shows both side-by-side so operators can distinguish "tool itself slow" from "prompt-bloat-tax slow."
    (b) **Parallel-tool-call instrumentation** — count how many turns emit ≥2 parallel `tool_use` blocks vs sequential. Claude Code SDK supports parallel tool calls (multiple `tool_use` in one assistant message); model has to choose. Add `parallel_tool_call_rate` per-agent metric.
    (c) **System-prompt directive nudge** — add fleet-wide directive (Phase 999.1 / Phase 999.22 pattern of locked-additive directives) instructing agents to issue parallel tool calls when reading multiple files / running independent searches. Test impact via sub-scope 16 benchmark.
    
    This directly attacks the observed 60-700s p95 tool latencies for tools that should run in milliseconds.
...
- [x] 115-03-PLAN.md — Wave 2: structural backbone (hard tier-1 budget + Tier 1/Tier 2 formal split + no-LLM tool-output prune)
- [x] 115-04-PLAN.md — Wave 2: cache-breakpoint placement reorder (static-then-dynamic stable-prefix)
- [x] 115-05-PLAN.md — Wave 2: lazy-load memory tools (4 new MCP tools) + Phase 95 dreaming as Tier 1 consolidation engine (D-10 hybrid policy)
...
1. ~~**Phase 115 Plan 08 producer regression**~~ — **RESOLVED 2026-05-11 via commit `a0f30a6`** (`fix(115-08): port producer call sites to production iterateUntilResult path`). Root cause was DEEPER than stale cache: two parallel session-handle implementations existed in source. Producer call sites lived in `session-adapter.ts:iterateWithTracing` (test-only path, invoked via `wrapSdkQuery` / `createTracedSessionHandle`) while production ran `persistent-session-handle.ts:iterateUntilResult` (no producer call sites). Surgical port of 4 call sites + sentinel test pinning both paths shipped during the Phase 116-00 wave. After next deploy, fleet `traces.db` populates split-latency columns from the very next turn forward. Anti-pattern memory captured: `~/.claude/projects/.../memory/feedback_silent_path_bifurcation.md`.

2. **`clawcode tool-latency-audit` CLI returns `Error: Invalid Request`** — investigation deferred until post-deploy reveals whether it shares root cause with #1 (now resolved) or is a separate IPC-handler bug. Direct SQLite read against the agent `traces.db` is the working fallback that the original 999.7 audit used.
...
- [x] 117-09 — Discord visibility: 💭 reaction + footer via `advisor:invoked` event (now level-aware — see 117-11)
- [ ] 117-10 — Migration cleanup: `CLAUDE.md`, `clawcode.example.yaml`, `CHANGELOG.md`, phase `SUMMARY.md`
- [x] 117-11 — **`/verbose` operator Discord toggle** — per-channel slash command (`/verbose on|off|status`) controlling visibility level. SQLite-backed channel state (table `verbose_channels`, similar pattern to `AdvisorBudget`). Default OFF (today's behavior — 💭 reaction + footer only when advisor fires). When ON, the assistant delivery to that channel inlines advisor advice block (NOT Q+A — RESEARCH §13.2 correction). Generalizes 117-09 from "always-on footer" to "visibility level." Files: `src/usage/verbose-state.ts` (new), `src/manager/daemon.ts` (IPC + slash dispatch), `src/discord/slash-commands.ts` (register `/clawcode-verbose`), `src/discord/bridge.ts` (consume level when assembling delivery). **SHIPPED 2026-05-13** — see `117-11-SUMMARY.md`. Test delta: +11 passing (6 VerboseState CRUD + 5 handleVerboseSlash), 0 regressions.
