# Phase 110 Plan 06 — Image Shim Wave 4 Rollout Log
...
>
> **Plan:** [.planning/phases/110-mcp-memory-reduction-shim-runtime-swap/110-06-PLAN.md](110-06-PLAN.md)
...
All six MUST be GREEN on clawdy before flipping `admin-clawdy.shimRuntime.image`. **Prereq 0 (NEW for image)** — search rollout (Plan 110-05) MUST be GREEN before image starts; image inherits the same fleet-wide deploy artifact (binary + IPC plumbing).

| #   | Prerequisite                                                                           | Verify command (run on clawdy)                                                                                            | Expected                                       | Status |
| --- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------- | ------ |
| 0   | Plan 110-05 (search rollout) GREEN — fleet stable on Go search shim ≥ 24h              | `cat .planning/phases/110-mcp-memory-reduction-shim-runtime-swap/110-05-ROLLOUT-LOG.md \| grep -E 'fleet-green'`         | one or more `fleet-green` decisions logged    | [ ]    |
| 1   | Daemon serves `list-mcp-tools` IPC method (Plan 110-01)                                | `clawcode ipc call list-mcp-tools '{"shimType":"image"}' \| jq '.tools \| length'`                                       | integer ≥ 1                                    | [ ]    |
...
| 3   | Loader resolves `shimRuntime.image: static` to the static command (Plan 110-02)       | Add a *fake* test agent override `shimRuntime.image: static` in scratch yaml; `clawcode show-mcp <fake-agent>`           | command line = `/usr/local/bin/clawcode-mcp-shim --type image` | [ ]    |
...
```bash
# On clawdy
PRE_PID=$(pgrep -af 'clawcode image-mcp' | grep -i admin-clawdy | awk '{print $1}')
...
```bash
pgrep -af 'clawcode image-mcp' | grep -i admin-clawdy   # expect: nothing
pgrep -af 'clawcode-mcp-shim --type image'              # expect: 1 PID under admin-clawdy claude proc
NEW_PID=$(pgrep -f 'clawcode-mcp-shim --type image' | head -1)
...
Trigger one `image_generate` tool call from admin-clawdy (via Discord channel or direct CLI prompt). Confirm:
...
pgrep -af 'clawcode image-mcp' | wc -l                  # expect: 0 (zero Node search shims fleet-wide)
pgrep -af 'clawcode-mcp-shim --type image' | wc -l      # expect: 11 (one per fleet agent)
```

### 4.3 Aggregate RSS measurement

```bash
ssh clawdy '/opt/clawcode/scripts/integration/measure-shim-rss.sh image'
# (or run locally on clawdy if you're already there)
/home/jjagpal/.openclaw/workspace-coding/scripts/integration/measure-shim-rss.sh image
```
...
| ----------------- | ------------------- | ------ | ----------------- |
| fin-acquisition   | `image_generate`        |        |                   |
| _agent 2_         | `image_generate`        |        |                   |
| _agent 3_         | `image_generate`        |        |                   |

### 4.5 /api/fleet-stats — full fleet on static

```bash
curl -s "localhost:<dashboard-port>/api/fleet-stats" \
  | jq '.mcpFleet[] | select(.name == "image") | {agent, runtime, rssBytes}'
```
...
- **No automatic fallback code path exists** — and Plan 110-06 inherits source-grep regression tests (`TestRegisterSourceContainsNoFallbackOrRetry`, `TestIntegrationNoSpikeArtifactsInSource`) to prevent future contributors from silently re-introducing one.
...
- RSS measurements at time of incident (use `scripts/integration/measure-shim-rss.sh image`)
...
*Scaffolded by Task 1 of Plan 110-06 on 2026-05-06. Phase 1 and Phase 2 sections are populated in real time as the operator drives the gates.*
