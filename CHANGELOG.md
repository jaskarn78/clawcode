...
- **Phase 117 — Claude Code advisor pattern (multi-backend scaffold; Anthropic complete)** (2026-05-13, source-complete; deploy operator-gated). Replaces the fork-based `ask_advisor` implementation (forks the agent's session under an Opus override every call — fresh-session boot + full-context input cost, zero advisor-side caching) with a provider-neutral `AdvisorService` interface and three backend slots. **Shipped (11 plans, 50+ tasks, ~130 new tests across 11 waves):** (1) `AnthropicSdkAdvisor` (default) — uses Claude Agent SDK's `advisorModel` option (`sdk.d.ts:4930`) to wire the Anthropic `advisor_20260301` server tool in-request with prompt caching; executor decides timing inside its own turn. Spread-conditional injection at SDK Options sites (both `createPersistentSessionHandle` production path and `iterateWithTracing` test path instrumented to prevent silent-path-bifurcation). Per-assistant-message observer emits `advisor:invoked` / `advisor:resulted` on `SessionManager.advisorEvents`; terminal `usage.iterations[]` charges `AdvisorBudget.recordCall` per `type:"advisor_message"` entry. (2) `LegacyForkAdvisor` — preserves today's fork-to-Opus behavior as the operator rollback path (`advisor.backend: fork`). `forkAdvisorConsult()` extracted from `daemon.ts` as a top-level primitive; `try/finally stopAgent` invariant covered by parity tests. (3) `PortableForkAdvisor` scaffold — interface-conformant stub that throws Phase 118 deferred error; not selectable in config (schema rejects `"portable-fork"` at parse time). (4) `src/llm/CompletionProvider` interface seeded for future non-Anthropic providers (Phase 118+); no implementations. (5) Config schema `defaults.advisor.{backend, model, maxUsesPerRequest, caching}` + per-agent `agents[].advisor` partial override (caching uses raw shape preserving operator-explicit-vs-omitted for per-field fall-through). Four module-level resolvers (`resolveAdvisorBackend/Model/MaxUsesPerRequest/Caching`) with per-agent → defaults → hardcoded-baseline fall-through. Same `clawcode reload` flow as Phase 110 `shimRuntime`. (6) IPC `ask-advisor` handler dispatches through `AdvisorService.ask` for fork-backend agents and short-circuits with an explanatory response for native-backend agents (executor calls advisor mid-turn; no IPC round-trip needed). Truncation + `recordCall` ordering single-sourced in `DefaultAdvisorService`. (7) Agent awareness — capability manifest now lists `Advisor (Opus)` bullet + `## Advisor protocol` prose block (source-of-truth in `src/advisor/prompts.ts::buildAgentAwarenessBlock`), so agents reach for the advisor on hard decisions. Fork-backend bullet carries a `(legacy fork backend — operator rollback path)` qualifier. (8) Discord visibility — 💭 reaction on the triggering message via per-turn `advisorEvents` listeners; single response-mutation point between `editor.flush()` and the delivery branches appends a footer (`*— consulted advisor (Opus) before responding*`, `*— advisor unavailable (<errorCode>)*`, or fenced advice block in verbose mode); `advisor_redacted_result` falls through to the plain footer (no plaintext leak even in verbose). No new threads — visibility is in-band; `subagent-thread` skill is untouched. (9) `/clawcode-verbose on|off|status` operator slash command — admin-only, ephemeral, per-channel; SQLite-backed state in `~/.clawcode/manager/verbose-state.db` (separate from `advisor-budget.db`); flips the bridge from footer-only to a fenced advice block (≤500 chars). (10) Operator rollback lever — flip any one agent to `fork` via `agents[].advisor.backend: fork` + `clawcode reload`; no daemon restart, no fleet impact. **Preserved contracts:** `ask_advisor` MCP tool schema unchanged (`src/mcp/server.ts:91`); `ask-advisor` IPC method name unchanged (`src/ipc/protocol.ts:168`); `AdvisorBudget` 10/day default unchanged; `ADVISOR_RESPONSE_MAX_LENGTH = 2000` truncation single-sourced; `subagent-thread` skill + `spawn_subagent_thread` IPC untouched; `escalation.ts` fork-to-Opus logic untouched. **Known issue (architectural deferral):** MCP `ask_advisor` tool still registers for native-backend agents — the MCP server has no per-agent identity at startup (`src/mcp/server.ts:170` — "no agent context"). Gating registration would require env-injection at the auto-clawcode MCP entry plus a new MCP-side resolver path; falls outside Phase 117 boundary. User-visible correctness is preserved by the IPC short-circuit (native-backend `ask_advisor` calls return an explanatory response pointing operators at `agent.advisor.backend: fork` for synchronous semantics). Cosmetic gap — tool appears in `tools/list` but is effectively a no-op for native agents. Resolution: follow-up plan that owns both pieces. **Deferred to Phase 118:** Full `PortableForkAdvisor` implementation (transcript extraction + `CompletionProvider` call); first `CompletionProvider` impl (likely `AnthropicDirectProvider`); removal of fork-based code (≥1 week post-`native` production rollout without rollback). **Production deployment NOT performed in this phase** — held per `feedback_no_auto_deploy` + `feedback_ramy_active_no_deploy`. Operator-gated manual smoke procedure documented at `.planning/phases/117-.../117-10-SMOKE.md`. Phase summary at `.planning/phases/117-.../117-SUMMARY.md`.
